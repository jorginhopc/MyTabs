...
var chords = (function(){
    ...
    var ChordBoxImage = function(name, chord, fingers, size) {
        ...
        var InitializeSizes = function() {
            ...
            //jml _imageHeight = (_boxHeight + _ystart + _fretWidth + _fretWidth);
            _imageHeight = (_boxHeight + _ystart + _fretWidth + (_fretWidth*1.2));
        ...
        var DrawChordBox = function() {
			...
			var pen = Pen(_foregroundBrush, _lineWidth*0.80);
            var totalFretWidth = _fretWidth + _lineWidth;

            for (var i = 0; i <= FRET_COUNT; i++) {
                //jml var y = _ystart + i * totalFretWidth;
				var y = _ystart + i * (totalFretWidth*1.2);
            ...
            for (i = 0; i < 6; i++) {
				...
				//jml _graphics.DrawLine(pen, x, _ystart, x, _ystart + (_boxHeight*1.25) - _lineWidth);
            }

            if (_baseFret == 1) {
                //Need to draw the nut
                var nutHeight = _fretWidth / 2;
                _graphics.FillRectangle(_foregroundBrush, _xstart - _lineWidth / 2, _ystart - (nutHeight*0.5), _boxWidth, nutHeight*0.5);
            }
        };
        ...
        var DrawChordPositions = function() {
            ...
            for (var i = 0; i < _chordPositions.length; i++) {
                ...
                if (relativePos > 0) {
					...
					var ypos = relativePos * (totalFretWidth*1.2) + yoffset-1;
        ...
        var DrawChordName = function() {
            ...
            if (_baseFret > 1) {
                ...
                _graphics.DrawString(_baseFret + "º", fretFont, _foregroundBrush, _xstart + _boxWidth + 0.4 * _fretWidth, _ystart - offset);
            }
        }
    ...
    var ReplaceChordElements = function() {
		...
		//	var divs = $('<div></div>').attr({id:"draggable"}).insertAfter(elt);
            //var canvas = $('<canvas></canvas>').attr({width:chord.getWidth(), height:chord.getHeight()}).insertAfter(elt);
			var canvas = $('<canvas></canvas>').attr({class:"draggable", width:chord.getWidth(), height:chord.getHeight()}).insertAfter(elt);
		  // var canvas = $('<canvas></canvas>').attr({width:chord.getWidth(), height:chord.getHeight()});
		  // var divs = $('<div></div>').attr({class:"draggable",width:chord.getWidth(), height:chord.getHeight()}).append(canvas).insertAfter(elt);
